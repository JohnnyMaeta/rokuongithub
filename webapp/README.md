...
> 🔰 **初めての方はこちら → [はじめての導入ガイド（超初心者版）](./はじめての導入ガイド.md)**
>
> 専門用語を最小限に抑え、つまずきポイントを全部織り込んだ画面操作レベルのガイドです。本READMEは技術的な背景や詳細を知りたい人向けの簡略版です。
...
### 6. 管理者サインイン & 保存先フォルダ名を登録（2分）

1. ブラウザで `https://<あなたのVercel URL>/admin.html` にアクセス
2. **「① Microsoftアカウントでサインイン」** をクリック
3. 自分のMicrosoftアカウントでサインイン
4. 同意画面で **「承諾」**
5. ✅ サインイン完了画面が出たら、続けて **「② 保存先フォルダ名を登録」** カードのテキストエリアに、子どもたちのドロップダウンに表示する**保存先フォルダ名**を **1行に1つずつ** 入力
   - 例（テーマや課題ごとに分けたいとき）:
     ```
     ラーメンについて
     町の良いところ
     1学期の感想
     ```
6. **「保存」** をクリック → 「✅ 保存しました」と出れば完了

### 7. 子どもたちにURLを共有

`https://<あなたのVercel URL>/` を子どもたちに共有するだけ。

子どもはサインイン不要で、子ども画面に表示される
- **保存先フォルダ名**: 先生が登録した一覧からドロップダウンで選択
- **グループ名・ペア名・個人名**: 自由入力（ファイル名の先頭になる）

を入れて録音 → アップロードすると、OneDriveの `録音くん保存フォルダ/[選んだフォルダ名]/[入力した名前]_[日時].mp3` に保存されます。

> **保存先フォルダ名を追加・変更したいとき**: いつでも `/admin.html` を開いてテキストエリアを書き換えて「保存」すればOK。再デプロイ不要。
...
└── 録音くん保存フォルダ/
    ├── history.csv          ← 全録音の履歴
    ├── ラーメンについて/
    │   ├── 1班_20260506_143025.mp3
    │   └── 田中・佐藤_20260506_143112.mp3
    └── 町の良いところ/
        └── 山田太郎_20260506_150033.mp3
```
...
|---|---|---|---|
| 2026-05-06 14:30:25 | 1班_20260506_143025.mp3 | ラーメンについて | https://... |
...
| 子どもの画面で「先生がまだグループを登録していません」と表示される | `/admin.html` の「② グループ名を登録」で1行ずつ入力して保存（手順6-5） |
| `/admin.html` でグループ保存時に「管理者としてサインインしてください」と出る | サインインしていないか、セッションクッキー切れ。同ページの「① サインイン」を実施 |
...
│   ├── auth-callback.js     # OAuth完了 → RT保存 + 管理者セッション発行
│   ├── upload-url.js        # 子どもがアップロード先URLを取得（グループ検証付き）
│   ├── groups.js            # グループ名リストの取得(GET)・保存(POST)
│   └── log-history.js       # history.csv 追記
├── lib/
│   └── graph.js             # トークン/セッション/グループ管理 + Graph API共通
├── index.html               # 子ども向け録音画面（グループはドロップダウン選択）
├── admin.html               # 先生向け設定画面（サインイン + グループ管理）
├── package.json
├── vercel.json
└── .env.example
```
