...
子どもはサインイン不要で、登録されたグループから**ドロップダウンで1つ選んで**録音 → アップロード → あなたのOneDriveの `録音くん保存フォルダ/[グループ名]/` に保存されます。

> **グループを追加・変更したいとき**: いつでも `/admin.html` を開いてテキストエリアを書き換えて「保存」すればOK。再デプロイ不要。
